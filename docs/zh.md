...
- 成员：
    ...
    #### 变量：`longopt`
    - 描述：长参数字符串。（例子：一个 `--help` 参数对应一个值为 `"help"` 的 `longopt`。）
    - 类型：`std::string`
    
    #### 变量：`withParam`
    - 描述：选择是否需要获取该参数的附加内容。（例子：如果需要获取一个类似 `-o example` 的选项，将其设置为 `true`。）
    - 类型：`bool`
...
- 公开成员：
    ...
    - 值：
        ...
        |----|------|
        | `0` | 命令成功发送。 |
        | `1` | 命令发送失败。`ISocket` 类中的 `sendto` 函数失败。 |
        | `4` | 非法的 IP 地址。 |
    ...
    - 值：
        ...
        |----|------|
        | `0` | 消息成功发送。 |
        | `1` | 消息发送失败。`ISocket` 类中的 `sendto` 函数失败。 |
        | `4` | 非法的 IP 地址。 |
...
- 描述：将 ANSI 编码的 `std::string` 转换为 UTF-8 编码的序列。
- 参数：
    ```cpp
    (
